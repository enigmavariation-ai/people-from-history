...
3. **Daily challenge** — one figure per UTC day, same for everyone (deterministic from date via `getDailyFigure`). One play per day: once you win or give up, you're locked out until tomorrow. Unlimited guesses + reveal during the round; flat `scoreGuess` (no multiplier). Day-streak counter persists in localStorage; bumps on win-if-yesterday-was-won, resets on loss or skipped day. Wordle-style share text with a 10-square reveal-bucket grid. No backend write — single-player, device-scoped — until the "claim profile" upgrade ships cross-device sync.
